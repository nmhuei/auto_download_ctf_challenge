...
- **ui** (compact shell, 2026-08-30): capture/render thật được dùng làm gate thay vì chỉ kiểm snapshot text. AppHeader UCS_ExOdia nén từ 3 dòng xuống **1 dòng** với context + rail 8 stage inline, timestamp chỉ hiện từ 100 cột để ưu tiên path/URL; menu truyền đúng `Console.width` thay vì probe lại terminal; full splash giảm **15 → 7 dòng** (compact hẹp **3 → 2 dòng**) và bỏ tagline/operation-label/READY lặp. `status` bỏ panel `GIẢI` rỗng, fake `+0 flags 24h`/baseline sparkline và blank row giữa category; progress + points/hoard/drafts gộp vào một dòng trong panel `TIẾN ĐỘ`, title chỉ giữ event + platform. Capture PTIT 35 challenge vừa đúng **45/45 dòng ở 120×45**, đồng thời 80/60 cột có max-width đúng 80/60, không overflow. `Logger.banner()` cũng bỏ tagline duplicate.
