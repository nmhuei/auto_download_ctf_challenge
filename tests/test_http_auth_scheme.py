...
from ctf_downloader.utils.http_client import HAS_CURL_CFFI
...
def test_same_host_different_port_does_not_receive_platform_credentials(force_browser):
    ...
    owner_thread.start()
    target_thread.start()
    if force_browser and not HAS_CURL_CFFI:
        pytest.skip("curl_cffi required for browser impersonation")
    try:
