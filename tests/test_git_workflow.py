...
def test_checkpoint_and_push_root_repo_without_meta(git_env):
    repo, remote, _info = git_env
    (repo / "notes.md").write_text("root notes\n", encoding="utf-8")
    status = GitWorkflowService.status(repo)
    assert status["dirty_files"] > 0
    assert status["current_branch"] == "main"

    pushed = GitWorkflowService.checkpoint_and_push(repo, message="update root")
    assert pushed["committed"] is True
    assert pushed["pushed"] is True
