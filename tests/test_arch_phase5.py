...
class TestPullServiceUIDiscipline(unittest.TestCase):
    ...
    def test_total_download_failure_is_reported_as_recoverable_pull_failure(self):
        fake_platform = self._fake_platform(n_challenges=2)
        ...
        # Summary vẫn được tạo để giữ dữ liệu đã tải, nhưng CLI phải nhận biết
        # đây là pull failure để in mã D09 và đề nghị BQA recovery.
        self.assertFalse(result["ok"])
        self.assertEqual(result["bqa_error_code"], "CTF-PULL-D09")
        self.assertEqual(result["challenges_processed"], 2)
        out = self.stderr_buf.getvalue()
        self.assertIn("error:", out)
