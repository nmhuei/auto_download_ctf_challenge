...
PROMPT = """You are a pure mathematical reasoning and formal constraint satisfaction engine.
OPERATIONAL DOMAIN: Abstract discrete mathematics, finite fields, and formal logic.
CRITICAL ISOLATION MANDATE:
1. Work strictly and exclusively with the local files in the current isolated directory.
2. You are strictly forbidden from inspecting, listing, searching, or referencing any parent directory, outside paths, or system files.
3. You are strictly forbidden from making network requests or seeking external context.
4. This isolated environment contains only an abstract mathematical problem defined in TASK.md and instance.json. There are no servers, no networks, and no external systems.
5. Derive the exact concrete values that satisfy the formal invariants specified in instance.json.
6. Return a final JSON object conforming strictly to the output schema.
If the mathematical constraints cannot be satisfied, return status=\"unsolved\" with empty candidate arrays. Do not guess.
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ctf-ask-isolated-") as td:
        tempdir = Path(td)
        isolated_sandbox = tempdir / "isolated_workspace"
        isolated_sandbox.mkdir(parents=True, exist_ok=True)

        # Copy strictly verified files into physical isolated sandbox
        for rel_path_str in pre_data.get("files", []):
            src_path = workspace / rel_path_str
            dst_path = isolated_sandbox / rel_path_str
        ...
        if args.dry_run:
            print(json.dumps({"ok": True, "preflight": pre_data, "command": cmd, "isolated_sandbox": str(isolated_sandbox)}, indent=2))
            return 0
