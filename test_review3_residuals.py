...
def _make_ws(root: Path, challenges=(("Web", "Alpha"), ("Web", "Beta"))):
    (root / "challenges.json").write_text(json.dumps({
        "ctf_info": {"title": "TestCTF", "url": "https://x.test",
                     "platform": "gzctf", "user": "me"},
    }, ensure_ascii=False), encoding="utf-8")
    for cat, name in challenges:
        d = root / cat / name
        (d / "writeup").mkdir(parents=True)
        (d / "metadata.json").write_text(json.dumps({
            "id": abs(hash((cat, name))) % 10000,
            "name": name.replace("_", " "),
            "category": cat,
            "points": 100,
            "status": {"schema_version": 2, "solve": "solved_by_me",
                       "flag": {"value": "TST{f}", "state": "correct"},
...
class TestRL2PruneKeepsForeignDirs(unittest.TestCase):
    def setUp(self):
        self._tmp = tempfile.mkdtemp(prefix="rl2_pack_")
    ...
    def test_foreign_dir_kept_with_warning_tool_stale_dir_pruned(self):
        from ctf_downloader.services.writeup_exporter import WriteupExporter

        _make_ws(self.ws)
        pack1 = WriteupExporter(self.ws).build_pack(out_dir=self.out)
        subs1 = sorted(d.name for d in pack1.iterdir() if d.is_dir())
        self.assertEqual(subs1, ["Web_Alpha", "Web_Beta"])

        # User drop tay thư mục notes + file vào pack dir
        foreign = pack1 / "my_notes"
        foreign.mkdir()
        (foreign / "keep.txt").write_text("user data", encoding="utf-8")

        # Beta mất điều kiện export giữa các lần chạy
        meta = self.ws / "Web" / "Beta" / "metadata.json"
        data = json.loads(meta.read_text(encoding="utf-8"))
        data["status"]["solve"] = "unsolved"
        meta.write_text(json.dumps(data), encoding="utf-8")

        warned = []
        with patch.object(logger_mod.Logger, "warning",
                          lambda msg, *a, **k: warned.append(msg)):
            pack2 = WriteupExporter(self.ws).build_pack(out_dir=self.out)

        # Dir LẠ: GIỮ nguyênvẹn + warning liệt kê
        self.assertTrue(foreign.is_dir(), "dir lạ của user bị xoá mất (R-L2)")
        self.assertTrue((foreign / "keep.txt").is_file())
        self.assertTrue(warned, "phải có Logger.warning liệt kê dir lạ")
        self.assertIn("my_notes", "\n".join(warned))

        # Stale dir CỦA TOOL (có trong manifest lần chạy trước): vẫn prune
        subs2 = sorted(d.name for d in pack2.iterdir() if d.is_dir())
        self.assertEqual(subs2, ["Web_Alpha", "my_notes"])
        import zipfile
        with zipfile.ZipFile(str(pack2) + ".zip") as z:
            dirs = {n.split("/")[1] for n in z.namelist() if n.count("/") >= 2}
        self.assertNotIn("Web_Beta", dirs, "stale dir của tool vẫn phải ra khỏi zip")

        # INDEX.md chỉ liệt kê entries hiện tại (không tham chiếu dir lạ)
        index = (pack2 / "INDEX.md").read_text(encoding="utf-8")
        self.assertNotIn("my_notes", index)
