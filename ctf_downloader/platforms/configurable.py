...
class ConfigurablePlatform(BasePlatform):
    ...
    def authenticate(self) -> bool:
        ...
        if endpoints.auth_check:
            url = urllib.parse.urljoin(self.base_url + "/", endpoints.auth_check.lstrip("/"))
        ...
        if endpoints.challenges:
            url = urllib.parse.urljoin(self.base_url + "/", endpoints.challenges.lstrip("/"))
            data, status = safe_get_json(self.session, url, statuses=(200,))
            if status == 200 and data is not None:
            ...
            if isinstance(files_raw, list):
                for f_item in files_raw:
                    if isinstance(f_item, str):
                        files_urls.append(urllib.parse.urljoin(self.base_url, f_item))
                    elif isinstance(f_item, dict):
                        f_url = f_item.get("url") or f_item.get("path") or f_item.get("link")
                        if f_url:
                            files_urls.append(urllib.parse.urljoin(self.base_url, str(f_url)))
    ...
    def submit_flag(self, challenge_id: Any, flag: str) -> Tuple[bool, str]:
        """Submit flag to configured submit endpoint."""
        endpoints = self.schema.endpoints
        ...
        if "://" in sub_path:
            sub_path = "/" + sub_path.split("://", 1)[1].split("/", 1)[-1]
        target_url = urllib.parse.urljoin(self.base_url + "/", sub_path.lstrip("/"))
        ...
        try:
            if method == "POST":
                ...
                if submit_spec.content_type == "json":
                    resp = self.session.request(method, target_url, json=payload, timeout=timeout)
                else:
                    resp = self.session.request(method, target_url, data=payload, timeout=timeout)
        except Exception as e:
            Logger.error(f"Lỗi gửi flag lên {self.schema.label}: {e}")
            self.last_verdict = "unknown"
            return False, str(e)

        if resp.status_code == 429:
            self.last_verdict = "ratelimited"
            Logger.warning(f"Bị rate-limit khi gửi flag trên {self.schema.label} (HTTP 429).")
        ...
        # Evaluate success with robust comparison DSL (==, !=, :, =)
        success = False
        succ_field = (submit_spec.success_field or "success").strip()
        expected_val = None
        op = "=="
        if "==" in succ_field:
    ...
    def fetch_scoreboard(self, if_none_match: Optional[str] = None) -> Dict[str, Any]:
        ...
        target_path = self.schema.endpoints.scoreboard.lstrip("/")
        if "://" in target_path:
            target_path = "/" + target_path.split("://", 1)[1].split("/", 1)[-1]
        target_url = urllib.parse.urljoin(self.base_url + "/", target_path.lstrip("/"))
        headers = {}
        if if_none_match:
